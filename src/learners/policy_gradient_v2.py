...
class PGLearner_v2:
    ...
    def _calculate_advs(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        ...
        for t in range(batch.max_seq_length):
            ...
            outputs = self.mixer.forward(q_out, batch["state"][:,t])

            for i in range(outputs.shape[0]):
                gradient = th.autograd.grad(outputs=outputs[i,0,0],inputs = q_out,retain_graph=True,allow_unused=True)

                grad = gradient[0][i]
                output_vector = th.zeros_like(grad)
                count+=1
                # Iterate over the elements of the input vector
                for i in range(len(grad)):
                    # Apply max(-val, 0) to each element and store the result in the output vector
                    output_vector[i] = max(-grad[i], 0)

                gradients.append(output_vector)
